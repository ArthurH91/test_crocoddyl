...
from collision import CostModelPairCollision
class OCPPandaReachingCol():
    ...
    def __init__(self, rmodel : pin.Model,cmodel : pin.GeometryModel, cdata, TARGET_POSE : pin.SE3, T : int, dt : float,  x0 : np.ndarray, WEIGHT_xREG = 1e-1, WEIGHT_uREG = 1e-4, WEIGHT_COL = 1e0, WEIGHT_COL_TERM = 1e-2, WEIGHT_GRIPPER_POSE =10) -> None:
        ...
        self._WEIGHT_COL_TERM = WEIGHT_COL_TERM
    ...
    self._state, self._rmodel.getFrameId("panda2_leftfinger"), self._TARGET_POSE.translation
        ...
        for k in range(len(self._cmodel.collisionPairs)):
            colcost = CostModelPairCollision(self._state, self._cmodel, self._cdata, k, 7, self._cmodel.geometryObjects[16+k].parentFrame)
            self._runningCostModel.addCost("col" + str(k), colcost, 10, self._WEIGHT_COL)
            self._terminalCostModel.addCost("col_term_" + str(k), colcost, self._WEIGHT_COL_TERM)
